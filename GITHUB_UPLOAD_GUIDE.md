...
- **bash**：一种命令行解释器（shell）的名称
  - 全称是 "Bourne Again SHell"
  - 是 macOS 和 Linux 系统的默认命令行界面
  - 在 Windows 上，Git Bash 提供类似的 bash 环境
  - 文档中 ` ```bash ` 标记表示这些命令需要在终端/命令行中执行

- **终端（Terminal）**：命令行界面，用于输入和执行命令
  - 在 Cursor 中可以通过菜单打开：Terminal → New Terminal
  - 或者在 Cursor 中使用快捷键（macOS: `` Ctrl + ` `` 或 `` Cmd + ` ``）

- **命令**：在终端中输入并执行的指令
  - 例如：`git init`、`git add .` 等
  - 命令通常由命令名 + 参数/选项组成

- **命令提示符**：终端中显示的位置标识
  - 通常以 `$` 或 `%` 结尾，前面显示当前目录
  - 例如：`~/Desktop/ece 422/Final_Project $`

- **代码块标记 ` ```bash `**：
  - 文档中使用 ` ```bash ` 来标记需要在终端中执行的命令
  - 这只是格式标记，**不是命令本身**
  - 实际使用时，只需要复制 ` ```bash ` 和 ` ``` ` 之间的命令内容
...
当您修改了代码后，想要再次上传更新，**只需要执行以下 3 个步骤**：

### 重要说明：哪些步骤是一次性的？

**只需要做一次的步骤**（首次设置完成后，以后都不需要再做了）：
- ✅ 步骤 1：在 GitHub 上创建仓库（已完成）
- ✅ 步骤 2：初始化本地 Git 仓库（已完成，`.git` 文件夹已存在）
- ✅ 步骤 3：配置 Git 用户信息（已完成，除非换账户）
- ✅ 步骤 6：添加 GitHub 远程仓库地址（已完成）
...
**步骤 1**：查看修改了哪些文件（可选，建议执行）
```bash
git status
```
- 显示哪些文件被修改了（红色）或已添加到暂存区（绿色）
...
**步骤 3**：创建提交并推送到 GitHub
```bash
git commit -m "描述本次修改的内容"
git push
```
- `git commit`：创建提交，记录本次修改
- `git push`：将提交推送到 GitHub（如果之前用过 `git push -u origin main`，现在直接 `git push` 即可）
...
```bash
git add .                              # 添加所有修改的文件
git commit -m "描述本次修改的内容"      # 创建提交
git push                                # 推送到 GitHub
```

就这么简单！后续每次修改代码，都只需要这 3 个命令。
